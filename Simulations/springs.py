"""the simulation generated by this script doesn't actually use most of the things in it, because they're incomplete or deprecated"""
...
class Rigidbody:
    def __init__(self, mass: qu.Mass, position: qu.Length, rotation: qu.Unitless[Vector], momentum: qu.Momentum, angular_momentum: qu.times[qu.Length[Vector], qu.Momentum[Vector]]):
        self.mass: qu.Mass = mass
        self.position: qu.Length = position
        self.rotation: qu.Unitless[np.ndarray] = rotation
        self.momentum: qu.Momentum = momentum
        self.angular_momentum: qu.times[qu.Length, qu.Momentum] = angular_momentum
        self.reset_added()

    def reset_added(self):
        self.added_force = qu.Force(Vector.zero())
        self.added_torque = qu.Length(1.) * qu.Force(Vector.zero())
        self.added_momentum = qu.Momentum(Vector.zero())
        self.added_angular_momentum = qu.Length(1.) * qu.Momentum(Vector.zero())
